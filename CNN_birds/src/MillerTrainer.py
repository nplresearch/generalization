...
class TrainerConfig:
    ...
    losses = ["reconstruction","similarity"]
    num_epochs: int = 100
    learning_rate: float = 1e-3
    weight_decay: float = 1e-5
    patience: int = 10  # epochs before reducing learning rate
    min_lr: float = 1e-6
    task_weights: Dict[str, float] = None  # weights for different tasks
...
class MillerTrainer:
    def __init__(
        ...
        # Set default task weights if none are provided
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch_idx in range(self.data_generator.config.num_samples // self.data_generator.config.batch_size):
            ...
            #sim_labels_inputs = self.data_generator.indices_to_inputs(sim_labels, self.data_generator.config.sequence_len - 1)
            
            # Move data to the configured device 
            sim_inputs = sim_inputs.to(self.config.device)
            #sim_labels_inputs = sim_labels_inputs.to(self.config.device)
            ...
            if "similarity" in self.config.losses:
                sim_outputs = nn.functional.relu(torch.bmm(embs[:,:-1,:],embs[:,[-1],:].permute((0,2,1)) )) + 10**-10
        ...
        metrics = {
            ...
            #'identification_loss': epoch_metrics['identification_loss'] / num_batches,
            'similarity_accuracy': epoch_metrics['similarity_correct'] / epoch_metrics['total_samples'],
            #'identification_accuracy': epoch_metrics['identification_correct'] / epoch_metrics['total_samples',
    ...
    def validate(self) -> Dict[str, float]:
        ...
        with torch.no_grad():
            ...
            g_score, i_score = evaluator.evaluate_model(self.model, self.data_generator)
    ...
    def train(self, verbose: bool = True, shutupfor = 10, compute_embeddings = False) -> Dict[str, List[float]]:
        ...
        for epoch in range(self.config.num_epochs):
            ...
            if verbose and epoch % shutupfor == 0:
                print(f"Epoch {epoch}:")
            ...
            # if epochs_without_improvement >= self.config.patience * 2:
    ...
    def _save_checkpoint(self, epoch: int, score: float):
        ...
        torch.save(
            checkpoint,
            f"{self.config.checkpoint_dir}/model_epoch_{epoch}_score_{score:.4f}.pt"
        )